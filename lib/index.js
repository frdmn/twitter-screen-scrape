...
(function() {
  ...
  getPostElements = function(username, startingId) {
    ...
    url = "https://twitter.com/i/profiles/show/" + username + "/media_timeline";
    // url = "https://twitter.com/i/profiles/show/" + username + "/with_replies";
    options = {
  ...
  TwitterPosts = (function(superClass) {
    ...
    TwitterPosts.prototype._read = function() {
      ...
      return getPostElements(this.username, this._minPostId).then(function(response) {
        ...
        return function($) {
          var action, element, hasEmitted, hasMorePosts, id, isRetweet, j, k, l, len, len1, len2, pic, pics, movie, movies, len3, post, ref, textElement, wrapper;
          hasEmitted = false;
          ref = $('.original-tweet');
          ...
          for (j = 0, len = ref.length; j < len; j++) {
            ...
            pics = $(element).find('.multi-photos .multi-photo[data-image-url], [data-image-url]');
            for (l = 0, len2 = pics.length; l < len2; l++) {
              pic = pics[l];
              post.images.push($(pic).attr('data-image-url'));
            }

            movies = $(element).find('.PlayableMedia-player');
            for (l = 0, len3 = movies.length; l < len3; l++) {
              movie = movies[l];
              var matches = $(movie).attr('style').match(/\'(.*?)\'/g);
              if (matches.length > 0) {
                var videoElement = matches[0].replace('\'', '').replace('\'', '').replace('https://pbs.twimg.com/tweet_video_thumb/', '').replace('\.jpg', '').replace('\.png', '');
                post.movies.push( 'https://pbs.twimg.com/tweet_video/' + videoElement + '.mp4');
              }
            }
            if (lastPost != null) {
              _this.push(lastPost);
